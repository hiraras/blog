...
```js
/**
 * @param jsUrl: 脚本地址（必须同源）
 * @param options(可选): 配置
 * {
 *  name：可以用来区分Worker，在Worker线程内可以使用 self.name获取，未指定时，获取到的是空字符串
 *  type：指定worker类型，可以是classic或module，默认为classic，module模式，可以让Worker线程导入其他脚本时使用 ESModule 模式
 *  credentials：指定 worker 凭证，可以是omit、same-origin、include，如未指定或type是classic，将使用默认值omit（不要求凭证）
 * }
 */
