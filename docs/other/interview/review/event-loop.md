...
-   我正在执行一个 js 函数，执行到一半的时候用户点击了按钮，我该去立即执行点击事件的处理函数吗？
...
-   网络通信完成后需要执行的任务 -- XHR、Fetch（网络线程实现）
...
3. 浏览器明明有渲染主线程/网络请求线程/计时线程/io 线程为什么还说 `js是单线程的`

这主要看 js 执行的时候是在单个线程中执行的，而网络请求线程/计时线程/io 线程这些线程是浏览器或 node 环境提供的功能，它们的作用是主线程将任务分给这些线程后，它们各自运行分配到的任务，在任务执行完之后，将回调扔会消息队列中，不参与后续执行了。消息队列中的任务什么时候执行跟任务进入队列的时间有关也跟 js 自己的调度有关。有个例外是 web worker，它会创建一个真正的 js 线程同步运行代码，但这属于创建线程，js 没有规定不能创建新的线程来工作
