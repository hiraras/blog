...
// exec: 类似match方法，值得注意的是，如果正则带了g，还是会返回单个子串的信息，再次执行会返回后续匹配到的子串，直到返回null，然后再从第一个字串开始循环
const str = "123";
const reg = /[a-z]/g;
reg.exec(str); // null
...
与普通遍历器的区别在于 next 返回一个 promise 对象，fulfilled 状态时 resolve 一个格式为{value: 1,done: false}的对象

## for await...of

一个对象的同步遍历器接口部署在`Symbol.iterator`属性上，而异步遍历器接口部署在`Symbol.asyncIterator`属性上

可以使用 `for await...of` 来自动遍历具有`Symbol.asyncIterator`接口的对象
...
```js
// 任何一个await语句后面的Promise对象变为reject状态，那个async函数都会中断执行
async function f() {
...
es2022 开始，允许在模块的顶层独立使用 await 命令
...
// usage.js
import { output } from "./x.js";

function outputPlusValue(value) {
    return output + value;
}

console.log(outputPlusValue(100)); // 还未加载output为undefined -> NaN
setTimeout(() => console.log(outputPlusValue(100)), 1000); // 101
```

顶层 await 只能用在 es6 模块，不能用在 CommonJS，因为 require 是同步加载，await 没法处理
...
setTimeout(() => console.log(outputPlusValue(101)), 3000);
...
console.log("Y");
/**
 * import是静态分析所以一开始就会执行，然后后续代码会等待其他模块的await执行完毕再执行
 * 以上代码输出顺序
 * 1. 立即输出"x start"
 * 2. 等待1s
 * 3. 依次输出 "Y" "index, start"  101
 * 4. 等待2s
 * 5. 输出 102
 */
```
...
es6 module 的模块导出语句，只要是写在模块的顶层就行，如果写在块级作用域内会报错。
...
// export输出的接口，与其对应的值是动态绑定关系，也就是说可以取到模块内部的值是实时的。
export var foo = 'bar';
...
使用 export 命令定义了模块的对外接口以后，其他 JS 文件就可以通过 import 命令加载这个模块。import 命令是编译阶段执行的，在代码运行之前
...
import "lodash"; // 重复导入不会报错，且只会执行一次
```
...
// 整体输出，注意！export * 命令会忽略模块的 default 方法
...
允许为 import 命令设置属性，主要用于导入非模块的代码，例如 json 数据、WebAssembly 代码、css 代码
...
传统的浏览器加载文件，会等待脚本下载完，并执行，期间造成浏览器堵塞。可以通过打开 `defer` 或 `async` 属性，使脚本异步加载
...
浏览器加载 es6 模块，需要设置 `type="module"` 属性，对于 `type="module"`的 script 标签都是异步加载
...
-   CommonJS 模块输出的是一个**值的拷贝**，ES6 模块输出的是值的引用。
...
```js
{
  "main": "./main-legacy.cjs", // 不支持es6的话，exports会被忽略
  "exports": {
    ".": "./main-modern.cjs" // 支持es6的话，因为优先级更高会覆盖main
  }
}
```
...
```js
import cjsModule from "../index.js"; // 同上面 CommonJS 模块加载 ES6 模块的方法
export const foo = cjsModule.foo; // 导出指定项
```
