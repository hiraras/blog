...
-   遍历完所有组件后得到完整的 wip 树
-   将 wip 树 commit 更新到界面(Fiber 树是虚拟 dom 的高级描述，不需要转为虚拟 dom)
-   所有节点的 alternate 都循环引用与之对应的新旧 Fiber，新的 wip 会被当作 current，而旧的 current 被当作后续的 wip
...
-   将 current 中的旧 Fiber 和 JSX 进行 reconcile(先执行旧 Fiber 和 JSX 的 diff，如果可复用则用 `wip.alternate`，不可复用则重建 Fiber)
-   遍历完所有组件后得到完整的 wip 树
-   将 wip 树 commit 更新到界面(Fiber 树是虚拟 dom 的高级描述，不需要转为虚拟 dom)
