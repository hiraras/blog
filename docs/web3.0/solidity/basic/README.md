...
1. 通过 import 语句导入合约文件，`import './Context.sol';`
2. 通过 is 关键字继承合约，如: `contract MyToken is Context`(Context 为一个合约，导入后可以直接使用 Context)
3. 父合约里的方法可以直接被使用
4. 父合约里的函数可以使用 override 重写(详见[solidity 函数](/web3.0/solidity/function/))
5. 父合约里的状态变量，被多个合约继承，其中一个子合约修改了状态变量，其他合约中的状态变量也会一起被修改。这是因为子合约实际上是扩展了父合约的功能，所有继承自父合约子合约都共享同一个状态变量
6. 状态变量无法在子合约中重写
...
事件是能方便地调用以太坊虚拟机日志功能的接口，可以有这两个用法

- 获取打印信息（可以在控制台看到参数信息）
- 外部应用程序可以监听事件，并在事件触发时接收通知
